...
4. Check Homebrew's troubleshooting guide`);
          ...
          // Rilevazione errore 'no new privileges' (italiano / inglese)
          const noNewPrivRegex = /(nessun\s+nuovo\s+privilegio|no\s+new\s+privileges?)/i;
          if (noNewPrivRegex.test(filteredError)) {
            resolve(`[Error] Sudo non può essere eseguito in questo ambiente (flag "no new privileges" attivo)

🔐 Il processo Electron è stato avviato con la protezione kernel PR_SET_NO_NEW_PRIVS: ciò impedisce a sudo di elevare i privilegi.

Possibili cause:
1. L'app è eseguita dentro un sandbox (Flatpak, Snap, Firejail, container, systemd-run con NoNewPrivileges=yes)
2. Avvio tramite wrapper di sicurezza che imposta il flag
3. Restrizioni di sicurezza aziendali / hardening

Come risolvere / aggirare:
• Avvia TermInA fuori dal pacchetto sandbox (esegui: npm install && npm start nella cartella del progetto)
• Se stai usando Firejail: avvia l'app senza firejail (firejail --noprofile ... oppure disabilita profilo)
• Se è un AppImage lanciato con integrazione Firejail, prova: FIREJAIL_DISABLE=1 ./TermInA*.AppImage
• In Flatpak / Snap non è possibile usare sudo: esegui i comandi privilegiati in un normale terminale di sistema
• Per sola analisi spazio disco puoi limitarti alla home senza sudo:
  du -ah ~ 2>/dev/null | sort -rh | head -n 20

Per trovare i 10 percorsi più pesanti senza blocchi permessi:
  sudo du -x -ah --threshold=100M / 2>/dev/null | sort -rh | head -n 10

Se ti serve davvero sudo dentro l'app, dovresti implementare un helper esterno (service) autorizzato via polkit invece di usare sudo diretto.
...
function createWindow() {
  ...
  // Avvia rilevazione ambiente sandbox / no_new_privs dopo caricamento iniziale
  detectAndNotifySandbox();
...
// Rileva se l'app è in un ambiente sandbox / container e se il flag no_new_privs è attivo
function detectAndNotifySandbox() {
  // Esegui solo su Linux (le condizioni attuali riguardano l'errore riportato)
  if (process.platform !== 'linux') return;
  
  const env = process.env;
  const indicators = [];
  if (env.FLATPAK_ID) indicators.push('Flatpak');
  if (env.SNAP) indicators.push('Snap');
  if (env.APPIMAGE) indicators.push('AppImage');
  if (env.FIREJAIL) indicators.push('Firejail');
  if (env.CONTAINER) indicators.push('GenericContainer');
  
  let noNewPrivs = false;
  try {
    const status = require('fs').readFileSync('/proc/self/status', 'utf8');
    const match = status.match(/NoNewPrivs:\s*(\d+)/);
    if (match && match[1] === '1') noNewPrivs = true;
  } catch (e) {
    // Ignora errori (non Linux / permessi)
  }
  
  // Ulteriori indicatori container
  try {
    if (require('fs').existsSync('/.dockerenv')) indicators.push('Docker');
    if (require('fs').existsSync('/run/.containerenv')) indicators.push('Podman');
  } catch (e) {}
